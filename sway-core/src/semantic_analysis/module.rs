    ...
    pub fn type_check(
        ...
        let submodules_res = module_eval_order
            ...
            .map(|eval_mod_name| {
                ...
                Ok((
                    name.clone(),
                    ty::TySubmodule::type_check(handler, ctx.by_ref(), engines, name.clone(), kind, submodule)?,
                ))
            })
        ...
        if ctx.experimental.new_encoding {
            ...
            match &kind {
                TreeType::Predicate => {
                    let mut fn_generator = auto_impl::AutoImplAbiEncodeContext::new(&mut ctx).unwrap();
                    let node = fn_generator.generate_predicate_entry(engines, main_decl.as_ref().unwrap()).unwrap();
                    all_nodes.push(node)
                }
                TreeType::Script => {
                    let mut fn_generator = auto_impl::AutoImplAbiEncodeContext::new(&mut ctx).unwrap();
                    let node = fn_generator.generate_script_entry(engines, main_decl.as_ref().unwrap()).unwrap();
                    all_nodes.push(node)
                }
                TreeType::Contract => {
                    // collect all contract methods
                    let contract_fns = submodules.iter()
                        .flat_map(|x| x.1.module.submodules_recursive())
                        .flat_map(|x| x.1.module.contract_fns(engines))
                        .chain(
                            all_nodes.iter().flat_map(|x| x.contract_fns(engines))
                        ).collect::<Vec<_>>();

                    let mut fn_generator = auto_impl::AutoImplAbiEncodeContext::new(&mut ctx).unwrap();
                    let node = fn_generator.generate_contract_entry(engines, &contract_fns).unwrap();
                    all_nodes.push(node)
                }
                TreeType::Library => {}
            }
...
impl ty::TySubmodule {
    ...
    pub fn type_check(
        ...
        parent_ctx.enter_submodule(mod_name, *visibility, module.span.clone(), |submod_ctx| {
            let module_res =
                ty::TyModule::type_check(handler, submod_ctx, engines, kind, module, module_eval_order);
