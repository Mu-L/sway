...
impl DebugWithEngines for TypeInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>, engines: &Engines) -> fmt::Result {
        ...
        let s = match self {
            Unknown => "unknown".into(),
            UnknownGeneric { name, trait_constraints } => format!("generic({}){:?}", 
                name.to_string(),
                engines.help_out(&trait_constraints.0)
            ),
...
impl TypeInfo {
    ...
    pub(crate) fn apply_constraints_to_type_argument (
        ...
        let new_type_info = type_info.apply_constraints(handler, engines, constraints).unwrap();
    ...
    pub(crate) fn apply_constraints(
        ...
        match self {
            ...
            TypeInfo::Array(type_argument, length) => {
                Ok(TypeInfo::Array(
                    self.apply_constraints_to_type_argument(handler, engines, constraints, type_argument),
                    length.clone())
                )
            }
            TypeInfo::Custom {
                ...
                let type_arguments = type_arguments.iter()
                    .map(|type_argument| {
                        self.apply_constraints_to_type_argument(handler, engines, constraints, type_argument)
                    }).collect();
            ...
            TypeInfo::Struct(r) => {
                ...
                Ok(TypeInfo::Custom {
                    ...
                    type_arguments: Some(s.type_parameters.iter().map(|x| {
